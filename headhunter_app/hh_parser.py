    ...
    if salary is None:
        ...
        try:
            ...
            if "USD" in salary:
                ...
                salary = [x for x in salary if x.strip()]
                if len(salary) > 1:
                    return int(salary[0] + salary[1]) * 57
                return int(salary[0]) * 57
            elif "KZT" in salary:
                salary = re.split('\D+', salary)
                salary = [x for x in salary if x.strip()]
                if len(salary) > 1:
                    return int(salary[0] + salary[1]) * 0.12
                return int(salary[0]) * 0.12
            elif "бел." in salary:
                salary = re.split('\D+', salary)
                salary = [x for x in salary if x.strip()]
                if len(salary) > 1:
                    return int(salary[0] + salary[1]) * 22
                return int(salary[0]) * 22
            else:
                salary = re.split('\D+', salary)
                salary = [x for x in salary if x.strip()]
                if len(salary) > 1:
